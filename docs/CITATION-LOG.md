...
| **Dell's only 800G→2×400G rail breakout is `DAC-O112-800G2x400G-Q112-xM`: OSFP112 → 2× QSFP112 far ends, "can plug into Broadcom 57608 NIC only; Supports 1x400 only"** — NOT "2× QSFP56-DD" as the catalog said until 2026-09-17 (GAPS G-034) | `corpus/txt/OPTICS.txt:1116-1126`, "800-Gigabit Ethernet Active Optical and Direct Attach Cable" table; §800GbE intro (`:154`): *"800G to 2x400G Q112 breakout cables (1-4M)"*. Corroborated: `corpus/txt/SONIC-COMPAT.txt:1687-1693` — *"Z9864F-ON port-to-Dell Broadcom Ethernet NIC connection … Dell Broadcom BCM57608 … Dell-branded passive copper DAC cables of 1m, 2m, 3m, or 4m"* | optic `brk-800g-2x400` (`media`, `farCage:'qsfp112'`, `nicOnly:'Broadcom 57608'`); `pickHostCable` Dell OSFP-leaf 400G branch; engine NIC-restriction verify line | 2026-09-17 | CURRENT. **Open ruling:** which NIC do Dell-stack XE9680-class deals actually carry (Broadcom 57608 vs ConnectX-7)? Until answered every Dell 400G rail line is verify-flagged with the restriction; an OSFP answer is a hard error (no Dell part). |
| Dell-stack XE9680 rail NIC = Broadcom Thor2 (BCM57608) in Dell's own AI fabric design — "two DAC connections onto their respective NIC ports on the Broadcom Thor2" | AI-NETGUIDE.txt:177 (Dell Enterprise SONiC AI Fabrics Networking Guide, H04600) | `engine.js` NIC-restriction warning + line note; `platforms.js` xe9680 note | 2026-09-18 | CURRENT — makes 57608 the EXPECTED NIC; the line stays verify-flagged (NIC is chosen on the server order) |
| Z9964F-ON / Z9964FL-ON EXIST: Tomahawk 6, 102.4 Tbps, 64× 1.6TbE OSFP224, 800GbE only as "128 (breakout)" | QRG-DC.txt:72-118 (June 2026) — the ONLY corpus document naming it | `switches.js` z9964f-on; `pickSpine` / `pickSuperSpine` gates | 2026-09-18 | CURRENT — real switch, GATED from 800G spine + super-spine picks until a 1.6T→2×800G OSFP112-far-end part is catalogued (see TASK row above) |
...
| Z-series AI switching capacity = QRG figure verbatim (Z9432F 12.8 / Z9664F 25.6 / Z9864F 51.2 / Z9964F 102.4 Tbps) — was printed doubled | QRG-DC.txt:80 (June 2026) | `switches.js` `switchingCapacity` (printed on BOM notes) | 2026-09-18 | CURRENT |
...
| Z9664F-ON power = 700 W, HELD above the QRG's "normal" 500 W | QRG-DC.txt:110 (June 2026) says 500; the 700 is untraced | `rules.power.switchWatts['z9664f-on']` | 2026-09-18 | RULED — a planning estimate may only err high; moves down only when a SECOND official Dell source agrees with 500 (re-rule trigger) |
| Z9432F-ON power = 900 W (normal) | QRG-DC.txt:110 (June 2026) | `rules.power.switchWatts['z9432f-on']` | 2026-09-18 | CURRENT (was an untraced 500 — under-estimated rack power) |
...
| Claim | Source doc | Enforced in | Verified | Status |
|---|---|---|---|---|
| **MX7000 external FSE uplink count = 8 per chassis** (4 Ethernet uplinks per MX9116n FSE × 2 FSEs in the A1/A2 HA pair). Per FSE: 2 dedicated 100GbE QSFP28 (ports 41-42) + 2 unified 100GbE QSFP28 (ports 43-44, Ethernet **or** native FC). The 12× QSFP28-DD (ports 17-40) are primarily FEM expansion but can also carry additional uplinks/VLTi — so 8 is the dedicated uplink-bank count, not a hard ceiling. | **TWO independent Dell documents, agreeing verbatim** — verified 2026-07-17: (1) **`corpus/txt/CO-MX-NET.txt` — H18548.9.2, "Dell Technologies PowerEdge MX Networking Deployment Guide", June 2026**: *"Two 100 GbE QSFP28 ports, used for Ethernet uplinks, ports 41 and 42 ● Two 100 GbE QSFP28 unified ports, used for Ethernet and Fibre Channel connections, ports 43 and 44"*; *"The two standard QSFP28 ports can be used for Ethernet uplinks. The QSFP28 unified ports can support Ethernet or native Fibre Channel connectivity"*. (2) **`corpus/txt/CO-MX-VCF.txt` — H19120, "Dell PowerEdge MX Deployment with VMware Cloud Foundation", March 2022**: *"In addition to 16 internal 25 GbE ports, the MX9116n FSE provides: ● Two 100 GbE QSFP28 ports ● Two 100 GbE QSFP28 unified ports ● Twelve 2x100 GbE QSFP28-Double Density (DD) ports"*. **NOTE FOR RECHECK: no standalone MX9116n spec sheet is known to exist** (aging modular platform; the port layout lives in the deployment guides). Do not reopen the spec-sheet hunt — these two guides ARE the authoritative citable source. | `platforms.js` mx7000 entry; `tests/unit-engine.js` G-006 regression block | 2026-07-17 | **CURRENT** — G-006 CLOSED. Prior `count:4` was a 2× undercount that contradicted the entry's own `portOptions` prose; fixed to 8 with a regression test that pins both halves of the arithmetic. **Caveat carried into the BOM:** the 2 unified ports/FSE are Ethernet-OR-FC — an FC-attached MX chassis flexes the count 8 → 4, surfaced as a rep-facing concern on the platform entry. |
...
| AI workload discovery pitch names the switches the engine actually picks (SN5600 / SN5610) | internal consistency check vs `engine.js`/`SPEC.md` | `discovery.js` workloads.ai | 2026-09-17 | CURRENT — the pitch already said "SN5600 / SN5610" when re-checked 2026-09-17 (this row had itself gone stale in the other direction after the content fix landed; G-036). The 2026-09-17 sweep also corrected the virtualization pitch's spine ("Z9432F-ON" → S5232F-ON / Z9264F-ON per the SPEC ladder) and the DFM "vendor-agnostic" claim (false — DFM manages Dell Enterprise SONiC only). |

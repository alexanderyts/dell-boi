...
| **TASK — 800G-native inter-switch rule.** "400G is the host/rail speed; inter-switch hops between same-cage switches run native port speed" is a DESIGN ruling (2026-07-16d), corroborated by part evidence (no cataloged OSFP→OSFP 400G assembly) but not yet by a Dell design doc. Verify against **H20082 / Dell AI Factory RA** when harvested. | H20082 is on the corpus want-list (`corpus/raw/` has AI-ERA + h04600, not H20082) | `engine.js` step 2 (`fs.uplinkSpeed` for AI); SPEC | — | **OPEN TASK** — **contradiction = re-rule trigger.** If the RA sizes 800G 3-tier differently, re-open the ruling rather than bending the code to both. |
| **TASK — super-spine 800G termination.** The Z9964F-ON (64× 1.6T OSFP224) is currently GATED out as a super-spine for 800G Z9864F-ON pod-spines: it reaches 800G only via breakout (`'800G': '128 (breakout)'`) and no 1.6T→2×800G part with **OSFP112 far-ends** is cataloged. Hunt for a real part in the Dell optics spec sheet + H20082/AI Factory RA. | Not found in `corpus/txt/OPTICS.txt` as of 2026-07-16 (no OSFP224 optic/breakout at any speed) | `superSpineTerminates` / `pickSuperSpine` (part-evidence gate) | 2026-07-16 (checked, absent) | **OPEN TASK** — the flagship is GATED, not banned: catalog a qualifying part and the Z9964F-ON re-enters the ladder automatically and the tier narrows. If the RA sizes this differently → re-rule trigger. |
...
| XE9680 = 1×400GbE rail/GPU | H20082, h04600 — **checked 2026-07-15: neither doc code appears anywhere in corpus**; `CO-XE-AI.txt` describes a different NIC config ("Embedded OSFP 6×800Gb (Gaudi3)" / "1× OCP 3.0") than "1×400GbE rail/GPU" | platform models | 2026-07-15 (checked, source not found) | NEEDS RECHECK (same unresolvable-source pattern as MX7000/G-006 — the cited doc codes don't exist in this repo) |
...
| Z9864F-ON "128 GPUs @ 400GbE rail-optimized" | H20082 — not found in corpus; the 128-GPU figure is a DERIVED number (64×800G ports × 2×400G breakout = 128 usable 400G rails), not a figure stated verbatim in any corpus doc | platform + engine | 2026-07-15 (checked — reasonable derivation, not directly sourced) | NEEDS RECHECK (the underlying hardware facts are confirmed; the "128 GPUs" framing itself is an inference, not a quoted spec) |
...
| Claim | Source doc | Enforced in | Verified | Status |
|---|---|---|---|---|
| MX7000: external FSE uplink count — is the intended total 4 (2/FSE) or 8 (4/FSE)? `portGroups[0].count: 4` conflicts with its own note "4× per FSE (2 FSEs/chassis)" | MX9116n FSE spec sheet — **checked 2026-07-15: does NOT exist in corpus/raw or corpus/txt** ("Fabric Switching Engine" full-text search across the entire corpus = zero hits; MX9116n/MX7116n appear only as unrelated optics-compatibility table row labels, no port-count text nearby) | `platforms.js` mx7000 entry (G-006) | 2026-07-15 (searched, source not found) | STALE — action needed (cited source was never actually harvested, or the citation is wrong; needs a real spec-sheet pull, not resolvable from this repo as-is) |
